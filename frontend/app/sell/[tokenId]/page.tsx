  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <section className="mx-auto w-full max-w-5xl px-6 py-6">
        ...
        <WalletGate>
          <SellMembershipContent tokenId={tokenId} />
        </WalletGate>
      </section>
    </main>
  );
...
function SellMembershipContent({ tokenId }: { tokenId: string }) {
  ...
  if (membershipsQuery.isLoading) {
    return <div className="rounded-lg border border-slate-200 bg-white p-6 text-slate-600">Loading membership...</div>;
  }
